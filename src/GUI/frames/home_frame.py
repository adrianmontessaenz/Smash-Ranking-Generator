"""Home frame and sub-frames for the GUI application.

Defines `HomeFrame` (the main landing page) and three internal
frames (`_LeftFrame`, `_MiddleFrame`, `_RightFrame`) used for the
left-side options, the central welcome text, and the information
tabs on the right.
"""
...
class HomeFrame(ctk.CTkFrame):
    """Main landing frame that composes left/middle/right children.

    Args:
        master: Parent widget (usually the root window).
        data_manager (DataManager): Data source passed to child frames.
    """
...
class _LeftFrame(ctk.CTkFrame):
    """Left-side option panel containing navigation and settings."""
    ...
    def _change_appearance_mode(self, new_mode):
        """Switch the global appearance mode (Light/Dark)."""
        ctk.set_appearance_mode(new_mode)

    def _save_api_key(self):
        """Persist the API key entered in the input box."""
        api_key = self.api_key_entry.get()
        self.master.data_manager.save_api_key(api_key)

    def compute_ranking(self):
        """Trigger the ranking computation using the provided DataManager."""
        compute_ranking(self.master.data_manager)
        
class _MiddleFrame(ctk.CTkFrame):
    """Center frame showing the application title and welcome text."""
    def __init__(self, master):
        super().__init__(master)
...
class _RightFrame(ctk.CTkFrame):
    """Right-side info tabs for quick access to data summaries."""
    def __init__(self, master):
        super().__init__(master)
